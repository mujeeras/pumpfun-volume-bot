...
const pumpFunSell = async (mint, amount) => {
  ...
  try {
    ...
    console.error(
      `Error executing sell transaction: ${error.message}`,
      error.response?.data
    );
    return null;
  }
...
const sellAllTokens = async () => {
  ...
  for (const token of tokens) {
    ...
    if (humanReadableAmount >= 1) {
      // Only proceed if human-readable amount is 1 or more
      ...
      while (attempts > 0) {
        ...
        if (txHash) {
          console.log(
            `Sold ${humanReadableAmount} of token ${mint} with transaction hash: ${txHash}`
          );
          break;
        } else {
          console.log(
